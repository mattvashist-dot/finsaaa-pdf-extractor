...
st.title("FINSA PDF → Excel (CSV) – v8 (Improved Quantity)")
st.caption("Upload up to 100 FINSA PDFs, extract structured data, and download one combined CSV.")
...
MONEY_RX = re.compile(r"\b\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})\b")       # 6,872.00 / 27,488.00 / 288.50
INT_OR_DEC_RX = re.compile(r"\b\d+(?:\.\d+)?\b")                        # 1 / 7 / 1.00 / 4.0 etc
UNIT_NEAR_QTY_RX = re.compile(rf"\b(\d+(?:\.\d+)?)\s+{UNIT_TOKENS}\b", re.I)

# ========================
# Totals: 3rd monetary line (your rule)
# ========================
def find_total_third_money_line(raw_text: str) -> str:
    """
    FINSA totals block pattern:
...
# ========================
# Quantity extraction — robust for wrapped rows
# ========================
def normalize_money_token(tok: str) -> str:
    """Normalize money format 6,872.00 / 6.872,00 -> 6872.00"""
    val = tok.replace(" ", "")
    if "," in val and "." in val:
        if val.rfind(",") > val.rfind("."):
            val = val.replace(".", "").replace(",", ".")
...
def split_into_item_rows(block_text: str) -> list:
    ...
    two money-like values (price & importe). We accumulate lines until we
    encounter a line with >= 2 monetary tokens, then we flush an item row.
    """
    if not block_text:
        return []
    ...
    for ln in raw_lines:
        ...
        if len(monies) >= 2:
            ...
            cur = []
    # if leftover content without 2 money values, attach to previous row
    if cur:
        if rows:
            rows[-1] += " " + " ".join(cur)
        else:
...
def infer_quantity_from_row(row_text: str) -> float | None:
    ...
    Given a consolidated row string:
      1) Prefer number immediately before a unit token (e.g., "7 PZA") → qty=7
    ...
    # 1) Number directly before a unit token
    ...
    # 2) Rightmost number before the first monetary token
    money_spans = list(MONEY_RX.finditer(row_text))
    if not money_spans:
        return None
    first_money_start = money_spans[0].start()

    # collect candidate numeric tokens before price
    candidates = []
    for nm in INT_OR_DEC_RX.finditer(row_text[:first_money_start]):
        tok = nm.group(0)
        try:
            v = float(tok)
            # quantities are usually not huge; allow integers or small decimals
            if v >= 0 and v < 100000:
...
def sum_quantities_advanced(block_text: str) -> str:
    """
    Split the item block into rows, infer quantity per row, and return sum.
    """
...
def parse_pdf(file_name: str, data: bytes, out_cols: List[str]) -> Dict[str, str]:
    ...
    if m_block:
        block = m_block.group(1) or ""
        # Build rows and decide multi vs single by row count
...
st.markdown("---")
st.caption("v8: Quantity parsing rebuilt — consolidates wrapped rows, detects price/importe per row, and sums quantities even across multi-line items. `item_id`/`item_desc` stay blank when multiple rows.")
